<div class="flex flex-col l:flex-row bg-amber-50 w-full fullscreen pt-2 lg:p-4 px-1 ">
    ...
    <div >
        ...
        @if ($method)        
            @livewire('admin.tablas.condiciones.modal',[ "method" => $method,"id"=>$id])
        @endif
          ...
          <div class="overflow-x-auto bg-white m-4 border-2 order-red-600 mx-auto rounded-md  shadow-md relative ">
            ...
            <x-action-message on="condicionCreated" class="bg-green-500   border-green-700 absolute left-0 z-10" >Condicion creada con exitó.</x-action-message> 
            <x-action-message on="condicionUpdated" class="bg-orange-500  border-orange-700 absolute left-0 z-10" >Condicion actualizada con exitó.</x-action-message> 
            <x-action-message on="condicionDeleted" class="bg-red-500  border-red-700 absolute left-0 z-10" >Condicion eliminada con exitó.</x-action-message> 
            <x-action-message on="condicionNotExits" class="bg-blue-500  border-blue-700 absolute left-0 z-10" >Condicion inexistente.</x-action-message> 
                        ...
                        <table class="min-w-full divide-y  divide-gray-400   p-1">  
                            <thead>              
                              <tr class="bg-gray-100 relative text-gray-600 font-bold divide-x-2 [&>th]:pl-2 [&>th]:pr-1 [&>th]:lg:pl-4 [&>th]:text-start text-sm ">                                  
                                <th scope="col" class="py-1">Condicion</th>
                            ...
                            <tbody class="divide-y divide-gray-200 text-gray-500  text-sm">
                              ...
                              <tr class="divide-x-2 [&>td]:pl-2 [&>td]:pr-1 [&>td]:lg:pl-4 [&>td]:text-start ">
                                <td class="py-1.5" >{{ $condicion->nombreCondicion }}</td>
                                <td> {{ $condicion->descripcionCondicion}}</td>       
                                <td  class="text-white">
                                    @if ($condicion->estado)
                                ...
                                <td >
                                  <div class="flex justfy-end lg:gap-x-6 gap-x-4 text-white text-xs">
                                    ...
                                    <button 
                                        class=" hover:text-gray-200  hover:bg-red-600 flex items-center py-0.5 bg-red-500 rounded-lg px-1 " wire:click="option('delete',{{$condicion->id}})">
                                    ...
                                    <button class=" hover:text-gray-200 hover:bg-orange-600 flex items-center py-0.5 bg-orange-500 rounded-lg px-1 " wire:click="option('update',{{$condicion->id}})" >
                                      <svg width="20px" height="20px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
