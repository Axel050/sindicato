          ...
          <div  class=" bg-white border w border-gray-500   md:max-w-md  lg:w-1/3 w-[96%] x-auto  z-50 rounded-lg shadow-gray-400 shadow-md max-h-[92%] overflow-y-auto " >
                ...
                <div class="bg-white  pb-6 text-gray-500  text-start ">
                    <div class="flex  flex-col justify-center items-center  relative">                             
                          ...
                          <x-action-message on="miembroReqUpdated" class="bg-green-500  border-green-700 absolute left-0 top-10  z-10" >datos actualizados con exitó.</x-action-message>                   
                          ...
                          <form class="bg-red-80  w-full  flex flex-col gap-2 lg:text-lg  text-base lg:px-4 px-2 text-gray-200  [&>div]:flex [&>div]:justify-center pt-4 "  wire:submit={{$method}} >
                            ...
                            @if ($method =="delete")
                                <p class="text-center text-gray-600 lg:px-10 px-6">Esta seguro de eliminar el requerimiento  </p>
                                <p class="text-center text-gray-600"><strong >"{{$requerimiento}}"</strong> de <strong >{{$nombre}}  </strong>?</p>
                              @else

                              <div class="flex flex-col   items-start  w-[90%] mx-auto ">
                                <label  class="w-full text-start text-gray-500  leading-[16px] text-base">Requerimiento </label>
                                <div class="relative w-full">
                                  <input type="text" wire:model="requerimiento" class ="h-7 rounded-md border border-gray-400 w-full text-gray-500" disabled/>                                  
                                </div>
                              </div>

                              <div class="flex flex-col   items-start  w-[90%] mx-auto ">
                                <label  class="w-full text-start text-gray-500  leading-[16px] text-base">Miembro </label>
                                <div class="relative w-full">
                              ...
                              <div class="flex flex-col   items-start  w-[90%] mx-auto ">
                                <label  class="w-full text-start text-gray-500  leading-[16px] text-base">DNI </label>
                                <div class="relative w-full">
                                  <input type="text" wire:model="dni" class ="h-7 rounded-md border border-gray-400 w-full text-gray-500" disabled/>                                  
                                </div>
                              </div>

                          @if ($requerimiento == "Datos actualizados")
                              <div class="flex flex-col   items-start  w-[90%] mx-auto mt-2  ">
                                <button type="button" wire:click="$set('methodModal',{{$condicion->user?->id}})" class="text-blue-500 underline hover:text-blue-700 mx-auto">Ver datos</button>
                              </div>
                                   @if ($methodModal)        
                                        @livewire('admin.requerimientos.modal-user',[ "id"=>$methodModal])
                                    @endif
                          @else
                              ...
                              <div class="flex flex-col   items-start  w-[90%] mx-auto mt-2  ">
                                ...
                                <div class="mt-4  w-full ">
                                    @if ($fileType === 'image')
                                        ...
                                        <img src="{{ $filePreview }}"   alt="Previsualización de imagen"  class=" max-w-52 rounded-md mx-auto">
                                    @elseif ($fileType === 'pdf')
                                        ...
                                        <div class="flex items-center space-x-2  w-full justify-center">
                                          ...
                                          <a href="{{ $filePreview }}" target="_blank" class="text-blue-500 underline hover:text-blue-700">
                                              Ver archivo PDF
                                          </a>
                                        </div>
                                    @else
                          ...
                          @if ($valor &&  !$filePreview)
                          <div class="mt-4 w-full text-center  flex justify-center">
                              @if (Str::endsWith($this->condicion->valor, ['.jpg', '.jpeg', '.png']))
                                  <!-- Mostrar la imagen -->
                                  <img src="{{ asset('storage/' . $this->condicion->valor) }}" alt="Archivo subido" class=" max-w-52 rounded-md mx-auto">
                              @elseif (Str::endsWith($this->condicion->valor, '.pdf'))
                                  ...
                                  <div class="flex items-center space-x-2">
                                      ...
                                      <a href="{{ asset('storage/' . $this->condicion->valor) }}"  target="_blank"  class="text-blue-500 underline">
                                          Ver archivo PDF
                                      </a>
            ...
            <div class="flex flex-col   items-start  w-[90%] mx-auto  mt-2 ">
              ...
              <div class="relative w-full">
                <select  wire:model.live="estado" class="w-full text-gray-700 h-7 rounded-lg py-0 border 
                @if($estado == 1) text-green-600  border-green-600 @elseif($estado == 2) text-orange-600 border-orange-600 @elseif(!$estado) text-red-600 border-red-600 @endif ">
                  <option value="0" class="text-red-600">Incompleto</option>
                  <option value="2" class="text-orange-600">Pendiente</option>
                  <option value="1" class="text-green-600">Aprobado</option>
                </select>
              </div>
            </div>
